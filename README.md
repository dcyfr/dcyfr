# Hi, I'm Drew ✨

<!-- README-META
  tlp_clearance: GREEN
  status: active
  name: dcyfr
  last_validated: 2026-05-03
-->

[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/dcyfr/dcyfr)

> Cyber Architect, Security Practitioner, and Agentic Builder.
> Founder of [**DCYFR Labs**](https://github.com/dcyfr-labs) and Founding Engineer at [**GameShark Labs**](https://github.com/gameshark-labs).

I design secure, agentic systems and the developer tools around them — currently focused on autonomous AI harnesses, plugin ecosystems, and cyber-architecture research.

## What I'm building

Most of my open work happens inside [**DCYFR Labs**](https://github.com/dcyfr-labs). The org's [profile](https://github.com/dcyfr-labs) has the canonical repo catalog, architecture diagram, and contributor guidelines — this page is the personal pointer.

**Highlights:**

- [`@dcyfr/ai`](https://github.com/dcyfr-labs/dcyfr-ai) — portable AI agent harness with plugin architecture
- [`dcyfr.ai`](https://www.dcyfr.ai) ([repo](https://github.com/dcyfr-labs/dcyfr-labs)) — research site, blog, and reference architecture (Next.js 16, React 19, MDX, Inngest)
- [Brand sites](https://github.com/orgs/dcyfr-labs/repositories?q=dcyfr-&type=public) — `dcyfr.{bot,io,app,tech,codes,build,work}`
- Starter templates for [agents](https://github.com/dcyfr-labs/dcyfr-ai-agents), [APIs](https://github.com/dcyfr-labs/dcyfr-ai-api), [GraphQL](https://github.com/dcyfr-labs/dcyfr-ai-graphql), [React](https://github.com/dcyfr-labs/dcyfr-ai-react), [chatbots](https://github.com/dcyfr-labs/dcyfr-ai-chatbot), and [Node.js](https://github.com/dcyfr-labs/dcyfr-ai-nodejs)

## Current focus

- DCYFR Labs plugin ecosystem and community marketplace
- Autonomous agent harness (`@dcyfr/ai`) — tool-use, memory, and routing
- Cyber-architecture research published on [dcyfr.tech](https://dcyfr.tech)

## Tech stack

![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Next.js](https://img.shields.io/badge/-Next.js-000000?style=flat-square&logo=next.js&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![GraphQL](https://img.shields.io/badge/-GraphQL-E10098?style=flat-square&logo=graphql&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Security](https://img.shields.io/badge/-Security-FF6B6B?style=flat-square&logo=shield&logoColor=white)

## GitHub stats

[![GitHub Streak](https://streak-stats.demolab.com?user=dcyfr&theme=github-dark&hide_border=true&short_numbers=true&date_format=%5BY%20%5DM%20j&card_width=600&card_height=200)](https://git.io/streak-stats)

## Connect

[![Homepage](https://img.shields.io/badge/-dcyfr.ai-000000?style=flat-square&logo=google-chrome&logoColor=white)](https://www.dcyfr.ai)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dcyfr/)

For security disclosures: **security@dcyfr.ai** (see [DCYFR Labs security policy](https://github.com/dcyfr-labs/dcyfr-ai/blob/main/SECURITY.md)).
